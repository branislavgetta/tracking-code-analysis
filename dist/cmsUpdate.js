...
const updateEntries = async () => {
    ...
    const tier0 = tiers.find((tier) => tier.tier === 0);
    const tier1 = tiers.find((tier) => tier.tier === 1);
    const filter = {
        _or: [
            {
                tier: {
    ...
    const eventNames = events.map((event) => lodash.camelCase(event.name));
    const contextsNames = contexts.map((context) => pascalCase(context.name));
